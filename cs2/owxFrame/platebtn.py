...
class PlateButton(wx.Control):
    ...
    def __init__(self, parent, id=wx.ID_ANY, label='', bmp=None, 
        ...
        self.Bind(wx.EVT_ENTER_WINDOW, self.__OnEnterWin)
                  #lambda evt: self._SetState(PLATE_HIGHLIGHT))
        self.Bind(wx.EVT_LEAVE_WINDOW, self.__OnLeaveWin)
    ...
    def __OnEnterWin(self, evt):
        self._SetState(PLATE_HIGHLIGHT)
        evt.Skip()
        
    def __OnLeaveWin(self, evt):
        evt.Skip()
        wx.CallLater(80, self.__LeaveWindow)
